...
use crate::eth_signer::eip712::definition::TypeDefinition;
        ...
        if let Some(type_name) = self.element_type_name()? {
            match type_name.as_str() {
                "bytes" => {
                    ...
                    let bytes = hex::decode(v).map_err(ser::Error::custom)?;

                    return self
                        .append_element(Keccak256::new().chain_update(bytes).finalize().into());
                }
                "address" => {
                    let address = v.parse::<Address>().map_err(ser::Error::custom)?;
